...
import { AppointmentReminder, ReminderType, ReminderStatus } from '../entities/appointment-reminder.entity';
...
export class ReminderService {
    ...
    async scheduleReminder(appointmentId: string, type: ReminderType, hoursBeforeAppointment: number = 24): Promise<AppointmentReminder> {
        const appointment = await this.appointmentRepository.findOne({ where: { id: appointmentId } });
        if (!appointment) {
            throw new Error('Appointment not found');
        }

        const scheduledTime = new Date(appointment.appointmentDate.getTime() - hoursBeforeAppointment * 60 * 60 * 1000);
