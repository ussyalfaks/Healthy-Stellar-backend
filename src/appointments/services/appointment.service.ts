...
export class AppointmentService {
    ...
    private async checkDoctorAvailability(doctorId: string, appointmentDate: Date, duration: number): Promise<boolean> {
    ...
    private calculateAvailableSlots(availability: DoctorAvailability, existingAppointments: Appointment[], date: Date): string[] {
        const slots: string[] = [];
        const [startHour, startMin] = availability.startTime.split(':').map(Number);
        const [endHour, endMin] = availability.endTime.split(':').map(Number);
        ...
        while (currentTime + availability.slotDuration <= endTime) {
            ...
            if (!hasConflict) {
                slots.push(`${String(Math.floor(currentTime / 60)).padStart(2, '0')}:${String(currentTime % 60).padStart(2, '0')}`);
            }
