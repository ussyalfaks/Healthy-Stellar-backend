...
var HttpExceptionFilter = function () {
    ...
    var HttpExceptionFilter = _classThis = /** @class */ (function () {
        ...
        HttpExceptionFilter_1.prototype.catch = function (exception, host) {
            ...
            if (status >= 500) {
                this.logger.error("".concat(request.method, " ").concat(request.url), exception instanceof Error ? exception.stack : JSON.stringify(exception));
            }
            else {
                this.logger.warn("".concat(request.method, " ").concat(request.url, " - ").concat(JSON.stringify(errorResponse)));
            }
