...
export class MedicalEmergencyErrorFilter implements ExceptionFilter {
  constructor(
    ...
    if (exception.name === 'EmergencyAlertError') {
      ...
      safeResponse = this.errorHandler.handleEmergencyAlert(
        exception.data,
        'EMERGENCY_ALERT'
      );
    } else if (exception.status === 400) {
      status = HttpStatus.BAD_REQUEST;
      safeResponse = this.errorHandler.handleValidationError(
        exception,
        'VALIDATION_ERROR'
      );
      ...
      safeResponse = this.errorHandler.handleMedicalRecordError(
        exception,
        'GENERAL_ERROR'
      );
    }
